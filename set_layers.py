...
def show_min_N(rho1, rho2, n_compute_method, lbd_center, show_comment, Rmin):
    n1 = n_compute_method(rho1)
    n2 = n_compute_method(rho2)
    R = W = n_cells = 0
    R_prev = R
    while R < Rmin:
        ...
        if R <= R_prev:
            print(f"Can't reach target reflectivity Rmin={Rmin}")
            break
    if show_comment:
        print(f"\nMin reflectivity reached for N={n_cells} -> R={R*100:.3f} %, W={W:.3f} %")
    return n_cells

def get_opt_porosities(lbd, n_compute_method=None, show_fct=False, comment_N=False, Rmin=0.95):
    ...
    if show_fct:
        ...
        x = y = np.linspace(p_min_max[0], p_min_max[1], 10)
        z = np.array([1/rho_to_R_inv([i,j], n_compute_method, lbd) for j in y for i in x])
        X, Y = np.meshgrid(x, y)
        Z = z.reshape(10, 10)
        plt.contourf(X, Y, Z, 100)
        plt.xlabel("p layer A")
        plt.ylabel("p layer B")
        plt.colorbar()
    ...
    res = minimize(rho_to_R_inv, start, method='L-BFGS-B', bounds=[p_min_max, p_min_max],
                   tol=tol, args=(n_compute_method, lbd),
                   options={'disp': True})
    p1, p2 = res.x  # [0.59, 0.75]
    N_min = show_min_N(p1, p2, n_compute_method, lbd, comment_N, Rmin)
    return p1, p2, N_min
...
if __name__ == '__main__':
    ...
    P_A, P_B, Nmin = get_opt_porosities(target_lbd, show_fct=True, comment_N=True, Rmin=0.95)
    solv = "air"
    N = 10
